...
| [`src/cli/`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/mod.rs) | The CLI-flags half of the compatibility surface: the clap-derived `Cli`/`Command` types for `run`, `inspect`, `cancel`, `kill`, `wait`, `list`, `prune`, and `probe`. Parsing and shape validation only — each subcommand's behavior lives in its own module. A directory of eight files, split by subcommand family rather than by size so a new flag or subcommand lands in a focused file: [`mod.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/mod.rs) holds the top-level `Cli`/`Command` shapes and re-exports everything below it, so `crate::cli::<Item>` stays the single path to every CLI type and parser; [`run.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/run.rs) holds `RunArgs`/`CaptureOverflowPolicy`; [`control.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/control.rs) holds `InspectArgs` and the `TargetArgs` shared by `cancel`/`kill`; [`wait.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/wait.rs), [`list.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/list.rs), [`prune.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/prune.rs), and [`probe.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/probe.rs) hold one argument struct each; and [`parse.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/parse.rs) owns the hand-written value parsers they share (durations, byte sizes, process counts, CPU quotas, exit-code bands, `KEY=VALUE` entries, run ids) — the functions the fuzz tier drives directly. |
