...
| [`src/cli/`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/mod.rs) | The CLI-flags half of the compatibility surface: the clap-derived `Cli`/`Command` types for `run`, `inspect`, `cancel`, `kill`, `attest`, `wait`, `events`, `list`, `prune`, and `probe`, plus the one global option every subcommand shares (`--error-format`, whose rendering lives in `src/error_envelope.rs`). Parsing and shape validation only — each subcommand's behavior lives in its own module. A directory of ten files, split by subcommand family rather than by size so a new flag or subcommand lands in a focused file: [`mod.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/mod.rs) holds the top-level `Cli`/`Command` shapes and re-exports everything below it, so `crate::cli::<Item>` stays the single path to every CLI type and parser; [`run.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/run.rs) holds `RunArgs`/`CaptureOverflowPolicy`; [`control.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/control.rs) holds `InspectArgs` and the `TargetArgs` shared by `cancel`/`kill`; [`attest.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/attest.rs) holds `AttestArgs` — a file of its own rather than a third user of `control.rs`'s shared target form, because `attest` deliberately takes only `--run-id`: no `--all` (it asks about one named run, never "am I inside *any* run") and no `--pid` or other way to name a process (the identity comes from the control connection itself, so the only question it can pose is "am I inside this run?"), which is the opposite of the target shape `cancel`/`kill` share; [`wait.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/wait.rs), [`events.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/events.rs) (whose implementing module is `src/events_cmd/`, the plain `events` name being taken by the emitter), [`list.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/list.rs), [`prune.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/prune.rs), and [`probe.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/probe.rs) hold one argument struct each; and [`parse.rs`](https://github.com/ZelAnton/ProcessKit-CLI/blob/main/src/cli/parse.rs) owns the hand-written value parsers they share (durations, byte sizes, process counts, CPU quotas, exit-code bands, `KEY=VALUE` entries, run ids) — the functions the fuzz tier drives directly. |
